...
def train(cfg, writer, logger, start_iter=0, model_only=False):
    ...
    # Setup Model
    model = get_model(cfg["model"], n_classes).cuda()
    ...
    if 'weight' in cfg['training']['loss']:
        cfg['training']['loss']['weight'] = torch.Tensor(cfg['training']['loss']['weight']).cuda(1)
            ...
            if (i + 1) % cfg["training"]["val_interval"] == 0 or (i + 1) == cfg["training"][
                ...
                with torch.no_grad():
                    for i_val, (images_val, labels_val, _) in tqdm(enumerate(valloader)):
                        images_val = images_val.cuda(2)
                        labels_val = labels_val.cuda(1)
